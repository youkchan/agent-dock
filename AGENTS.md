...
- レビュー完了判定は `全体監査 → 修正 → 全体再監査` を **1サイクル** 実施した場合のみ行う（部分確認のみで完了にしない）。
- 不具合指摘は各項目の先頭で、種別を必ず明示する（`文書` / `コード` / `両方`）。
- 要素を列挙する回答をする時（不具合、手順など）は必ずそれぞれの要素の行の間に改行を入れること
- Think hard：結論に至る推論を整理し、論点・前提・反証可能性を意識する。
- Search aggressively：記憶に頼らず、常に該当するソースコードやドキュメントを参照した上で回答すること
