...
- `target_paths` は「必須対象（実処理に直結）」として定義し、最小追加修正は `関連許可` に明記した範囲で許可する。
- `関連許可` を使って修正した場合は、RESULT の SUMMARY に `additional_edit_reason=<reason>` 形式を必ず記録する（自由記述のみは不可）。
- scope 判定は canonical path 比較を前提とし、`..` を含む相対パス・絶対パス・traversal を reject する。
...
## 1. 実装タスク
- [ ] 1.1 `agent-dock spec-creator polish` の仕様収集対象を実装する
  - 依存: なし
...
- [ ] 1.2 Polish 用プロンプトの組み立てを実装する
  ...
  - フェーズ担当: implement=implementer; review=code-reviewer
  - 成果物: 既存コンテキスト（対象 change の全 `.md`）＋`--feedback`＋最新基準（5行契約、JUDGMENT必須、判定3値化）を1回呼び出しに集約し、再生成対象（`proposal.md` `tasks.md` `code_summary.md` `specs/**/spec.md` + 必要時 `design.md`）として渡す。
  - 完了ゲート: 配線ゲート（生成プロンプトが実実行で使用されること）

- [ ] 1.3 SpecCreator に `polish` メソッドを追加し Codex 出力をパースする
  - 依存: 1.2
  - 対象: src/infrastructure/openspec/spec_creator.ts, src/infrastructure/openspec/spec_creator_test.ts
  - 関連許可: src/cli/main.ts, src/cli/main_test.ts
  - フェーズ担当: implement=implementer; review=code-reviewer
  - 成果物: Codex のマルチファイル出力をパースし、再生成対象（`proposal.md` `tasks.md` `code_summary.md` `specs/**/spec.md` + 必要時 `design.md`）を上書きするロジックを実装。
  - 完了ゲート: 回帰ゲート（パース正常系/異常系のテスト追加）

- [ ] 1.4 `agent-dock spec-creator polish` コマンドを追加する
  - 依存: 1.3
...
- [ ] 1.5 polish 実行後の `openspec validate` を自動実行する
  - 依存: 1.4
...
- [ ] 1.6 コンパイルエラー／実行時エラーの使い分けを含む最終整合レビューを行う
  ...
  - 成果物: 入力コンテキスト（全 `.md`）と再生成対象（`proposal.md` `tasks.md` `code_summary.md` `specs/**/spec.md` + 必要時 `design.md`）の分離を確認し、`implement` 必須化・3値判定・`(none)` ルール・CHANGED_FILES整合を確認する。
